    ...
    public class Searcher
    {
        ...
        private static void SearchThread()
        {
            ...
            if ((m_pars.SearchDir.Length >= 3) && (Directory.Exists(m_pars.SearchDir)))
            {
                if (m_pars.FileNames.Count > 0)
                {
                    ...
                    if (m_pars.ContainingChecked)
                    {
                        if (m_pars.ContainingText != "")
                        {
                            ...
                            {
                                success = false;
                                errorMsg = "Строка\r\n" + m_pars.ContainingText + "\r\nне может быть преобразована в байты.";
                            }
                        }
                        else
                        {
                            success = false;
                            errorMsg = "Строка для поиска не должна быть пустой.";
                        }
                    }
                ...
                {
                    success = false;
                    errorMsg = "Пожалуйста, введите одно или несколько имен файлов для поиска.";
                }
            }
            else
            {
